...
func TestAppHandlerServesStaticFilesAndSPA(t *testing.T) {
	indexDir := t.TempDir()
	distDir := filepath.Join(indexDir, "apps", "lite", "dist")
...
func TestResolveLiteDistDirFindsRepoRootFromWorkingDirectory(t *testing.T) {
	root := t.TempDir()
	...
	workDir := filepath.Join(root, "apps", "cloud")
	...
	resolved, err := resolveLiteDistDir()
	if err != nil {
		t.Fatalf("resolve lite dist: %v", err)
	}
...
func TestResolveLiteDistDirUsesEnvOverride(t *testing.T) {
	distDir := t.TempDir()
	if err := os.WriteFile(filepath.Join(distDir, "index.html"), []byte("index"), 0o644); err != nil {
		t.Fatalf("write index: %v", err)
	}

	t.Setenv(liteDistEnv, distDir)
