...
describe('cloud runtime defaults contract', () => {
  ...
  it('documents and falls back migrated discovery runtime defaults as backend-owned same-origin endpoints', async () => {
    ...
    const envLocalExample = readFileSync(
      resolve(process.cwd(), 'public/env.local.js.example'),
      'utf8'
    )

    expect(envJs).toContain('backend-owned and same-origin')
    expect(envJs).toContain('production Cloud')
    expect(envJs).toContain('dev/default-template')
    expect(envJs).toContain(
    ...
    expect(envLocalExample).toContain('generated by `apps/cloud-api`')
    expect(envLocalExample).toContain('frontend-only development')
