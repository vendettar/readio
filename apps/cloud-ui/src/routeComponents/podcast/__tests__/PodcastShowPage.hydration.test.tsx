...
import type { Podcast } from '../../../lib/discovery/providers/types'
...
describe('PodcastShowPage hybrid hydration (instruction 023/025)', () => {
        ...
        http.get(
          ...
          ({ request }) => {
            ...
            return HttpResponse.json({
              ...
              genres: [{ name: 'Technology', genreId: '1' }],
              episodeCount: 15,
  ...
  describe('hydration applied - route id fallback when lookup payload omits itunes identity', () => {
    beforeEach(() => {
      server.use(
        http.get(
          'http://localhost:3000/api/v1/discovery/podcast-index/podcast-byitunesid',
    ...
    it('should hydrate from the route id when the lookup payload omits itunes identity', async () => {
      render(<PodcastShowPage />, { wrapper: createWrapper() })
  ...
  describe('hydration applied - fallback to route id when podcastItunesId is absent', () => {
    beforeEach(() => {
      server.use(
    ...
    it('should fall back to the route iTunes id when podcastItunesId is absent', async () => {
      render(<PodcastShowPage />, { wrapper: createWrapper() })
  ...
  describe('hydration cache invalidation when trackCount drops', () => {
    ...
    it('should stop using stale hydrated episodes when trackCount drops below the hydration threshold', async () => {
      ...
      await queryClient.invalidateQueries({
        queryKey: buildPodcastIndexLookupQueryKey('123', 'us'),
        refetchType: 'active',
      })
      await queryClient.invalidateQueries({
