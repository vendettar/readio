...
export function useCarouselLayout(
  ...
  const calculateItemWidth = useCallback(() => {
    ...
    if (viewportWidth < effectiveFixedBreakpoint) {
      ...
      // DYNAMIC MODE: Find the largest number of items that fit within minItemWidth
      newVisibleCount = minVisibleItems // Fallback

      for (let c = maxVisibleItems; c >= minVisibleItems; c--) {
        const potentialWidth = (containerWidth - (c - 1) * gap) / c
  ...
  // Handle scroll event for real-time button updates
  useEventListener('scroll', updateScrollButtons, scrollRef, { passive: true })
  ...
  }, [calculateItemWidth, updateScrollButtons])
