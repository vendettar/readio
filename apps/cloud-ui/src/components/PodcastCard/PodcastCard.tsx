...
export function PodcastCard({
  ...
  const resolvedParams = params ?? (to ? { id } : undefined)
  ...
  return (
    <div className={containerClasses} data-testid="podcast-card">
      <div
        className={cn(
          'absolute inset-0 opacity-0 group-hover/card:bg-accent/50 transition-colors pointer-events-none',
          radiusClass
        )}
      />
      <div className={cn('relative aspect-square w-full', radiusClass)}>
        <InteractiveArtwork
          ...
          params={onClick ? undefined : resolvedParams}
          search={onClick ? undefined : search}
          state={onClick ? undefined : state}
          hoverGroup="card"
          className={cn(
        ...
        {rank !== undefined && variant !== 'circular' && (
          <div className="absolute bottom-2 start-2 w-7 h-7 bg-background/80 text-foreground backdrop-blur-md rounded-lg flex items-center justify-center border border-border/50 group-hover/card:opacity-0 group-hover/card:invisible transition-all duration-300 pointer-events-none shadow-sm">
