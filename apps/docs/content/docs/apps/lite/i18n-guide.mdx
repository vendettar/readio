...
- **Translations Object**: `{ "episode_count": "{{count}} Episode", "episode_count_plural": "{{count}} Episodes" }`
...
To prevent UI regressions caused by missing keys, we run a coverage audit script during the build process.
...
pnpm --filter @readio/lite i18n:check
```

The script (`apps/lite/scripts/audit-i18n.ts`) compares all languages against `en`. If there are missing keys, the build will fail.
