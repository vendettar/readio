...
- [ ] Mini/Full Player subscribe to store fields with selectors only; avoid full-store subscription for high-frequency fields (progress/time/scroll/drag).
- [ ] Immersion mode is controlled by `src/store/playerSurfaceStore.ts` (state-driven); `PlayerSurfaceFrame` renders `mode='full'` when expanded.
- [ ] Full mode can correctly exit (`toDocked()` or `toMini()`), returning to docked or mini state.
- [ ] `ReadingContent` MUST remain mounted during `docked <-> full` transitions to preserve transcript position and state.
...
- [ ] **Selection Anti-Regression**: 
  - [ ] Double/triple clicks are blocked via `mousedown` capture (`e.detail >= 2`).
  - [ ] Word/Range menus use stable **geometric anchors**.
  - [ ] Line empty-space menus use **pointer-origin anchors**.
  - [ ] All menus are **non-modal** (No blocker, click-through absorption).
  - [ ] Menus prioritize **top placement** to avoid text occlusion.
  - [ ] **Surface Switch Isolation**: Right-clicking between words/menus preserves the "paused" state without audio blips.
  - [ ] **Focus Restoration**: Focus returns to the word owner on dismiss (Left-click outside/Esc/Close) but is correctly skipped during word-to-word switch.
...
- [ ] `src/main.tsx` only imports global styles that are still in use (Current: `src/index.css`).
...
- [ ] `pnpm --filter @readio/lite test:run` (Verify `SelectionUI.positioning.test.tsx` and `useSelectionEvents.no-geometric-lookup.test.ts`)
- [ ] `pnpm build`
- [ ] Manual test:
  ...
  - [ ] ASR: Trigger successful generation on a short local audio file; verify failed state / error UI visible on repeated rapid triggers (queue protection) and offline/error visual state fallback occurs on failure.
