...
- [ ] Standardized DB using Dexie: `src/lib/dexieDb.ts` (DB name: `readio-v2`).
...
- [ ] `src/lib/__tests__/dbMigration.test.ts` focuses on initialization and CRUD assertions during the "clearable data" phase (No data persistence assertions).
...
- [ ] New/Modified UI text must use `t(key)` via `useI18n` (react-i18next adapter).
