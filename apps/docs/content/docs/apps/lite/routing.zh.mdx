...
| 路由 | 模式 | 描述 |
|---|---|---|
| Home | `/` | 根据状态重定向到 `/explore` 或 `/files`。 |
| Podcast | `/podcast/$id` | 播客详情页。`$id` 是 Provider ID (例如 iTunes ID)。 |
...
- **预加载**: 在路由定义中使用 `loader` 在渲染组件组件 *之前* 获取 Podcast Feed 数据。
...
- **法律页面路由**：`/legal/privacy` 与 `/legal/terms`（指令 039）。
