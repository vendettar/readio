...
- **Audio Blobs**: Object URLs MUST be created and revoked inside the store only (e.g. `playerStore.loadAudioSource(blob)`). Components must not call `URL.createObjectURL` for audio.
- **Image Blobs**: Components MUST create Object URLs for artwork using a hook that revokes on unmount (e.g. `useImageObjectUrl`).
