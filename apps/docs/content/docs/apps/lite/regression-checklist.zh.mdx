...
> 原则：基于 **实际当前代码** 和 `apps/docs/content/docs/general/technical-roadmap.mdx`；过时的项目应及时更新/删除。
...
- [ ] **每次更改后更新 `apps/docs/content/docs/apps/lite/handoff/index.mdx`** (仅描述实际当前状态；无过程说明)。
- [ ] **用户消息不得暴露技术术语** (CORS, HTTP 状态码, 堆栈跟踪去控制台/日志；UI 仅使用自然语言)。
- [ ] UI: 使用 Tailwind + shadcn/ui tokens；不要发明新 token (见 `apps/docs/content/docs/general/design-system/index.mdx`)。
...
- `src/components/AppShell/PlayerSurfaceFrame.tsx`
- `src/components/AppShell/ReadingContent.tsx`
...
- [ ] Mini/Full Player 只用选择器订阅 store 字段；避免对高频字段（progress/time/scroll/drag）进行全量订阅。
- [ ] 沉浸模式由 `src/store/playerSurfaceStore.ts` 控制 (状态驱动)；当进入全屏时，`PlayerSurfaceFrame` 以 `mode='full'` 渲染。
- [ ] Full 模式可以正确退出 (`toDocked()` 或 `toMini()`)，返回 docked 或 mini 状态。
- [ ] 在 `docked <-> full` 切换过程中，`ReadingContent` 必须保持挂载，以维持逐字稿位置和状态。
...
- [ ] Media Session 处理器：`src/hooks/useMediaSession.ts`、`src/components/AppShell/GlobalAudioController.tsx`。
...
- [ ] 视图缩放或变化后，选区、词典浮层和高亮位置保持正确。
- [ ] **选区与菜单防回归**:
  - [ ] 容器级 `mousedown` 拦截双击选词行为 (`e.detail >= 2`)。
  - [ ] 单词与划词菜单使用稳定的**几何中心锚点**。
  - [ ] 行内空白右键使用 **pointer-origin (点击点) 锚点**。
  - [ ] 所有菜单均为 **non-modal**（无全屏阻挡，支持点击穿透吸收）。
  - [ ] 菜单优先使用 **top placement** 以避免遮挡下方正在阅读的字幕。
  - [ ] **Surface 切换隔离**：在不同单词间右键切换菜单时，播放状态保持静止，无声音漏出。
  - [ ] **Focus 恢复规则**：在 Dismiss（左键点外部/Esc/关闭按钮）时恢复 focus 到单词，但在 Switch（单词间右键连续点击）时正确跳过 focus 恢复。
  - [ ] 鼠标在字幕行文字之间的空白处双击时**绝不能**出现系统蓝底文字选中框（即 `capture: true` `e.preventDefault()` 必须生效防止原生拦截）。
...
- [ ] 使用 Dexie 标准化 DB：`src/lib/dexieDb.ts` (默认 DB 名：`readio`，清库时可通过 `READIO_DB_NAME` 覆盖)。
- [ ] `DB.clearAllData()` 必须是表级清理 (事务 + `table.clear()`)；不要使用 `db.delete()`。
- [ ] 测试隔离：`fake-indexeddb` + 每个测试套件前清除 DB。
- [ ] `src/lib/__tests__/dbOperations.test.ts` 专注于“可清除数据”阶段的初始化和 CRUD 断言 (无数据持久性断言)。
...
- [ ] 新/修改的 UI 文本必须使用 `t(key)`（使用 `useTranslation()`，基于 react-i18next）。
...
- [ ] 离线状态：`src/hooks/useNetworkStatus.ts`、`src/routeComponents/ExplorePage.tsx`、`src/components/GlobalSearch/CommandPalette.tsx`、`src/components/AppShell/Sidebar.tsx`。
...
- [ ] `src/main.tsx` 仅导入仍在使用的全局样式 (当前：`src/index.css`)。
...
- [ ] `pnpm --filter @readio/lite test:run` (验证 `SelectionUI.positioning.test.tsx` 与 `useSelectionEvents.no-geometric-lookup.test.ts`)
- [ ] `pnpm build`
- [ ] 手动测试:
  - [ ] 5 个主要侧边栏路由正确打开：`/explore`, `/subscriptions`, `/favorites`, `/history`, `/files`。
  - [ ] 播放器：播放不受路由/沉浸更改的中断。
  - [ ] 逐字稿：多行可见性 + 跟随 + 选择查词 + 高亮。
  - [ ] ASR：测试生成首个本地音频的字幕；快速连续点击触发验证时，正确呈现错误状态/限流视觉提示（防爆器），且在无网/断网时正确呈现无侵入的失败视觉兜底。
