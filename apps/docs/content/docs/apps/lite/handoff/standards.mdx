...
When a new PWA version is available, the app shows an **infinite duration** toast with an action button to "Refresh now". This is managed by the `usePwaUpdate` hook, which uses the `needRefresh` state from `vite-plugin-pwa`.

#### Onboarding & Empty States
Blank screens are replaced with the `<EmptyState />` component which provides:
- A large visual icon.
- A prominent title and descriptive secondary text.
- A primary Call-to-Action (CTA) button to guide the user (e.g., "Discover Podcasts" or "Add Audio").

#### Asset Loading & Fallbacks
- **Image Fallback Policy**: Broken artwork URLs are automatically replaced by a Lucide `Podcast` icon centered in a themed placeholder.
- **Lazy Loading**: Images use native `loading="lazy"` or Framer Motion entrance animations where appropriate.

#### Tooltip

Provider is located at `src/main.tsx` (`Tooltip.Provider`).

### Accessibility (Immersive & Inclusive) [Implemented]

The application follows a strict accessibility standard to ensure usability for screen readers and keyboard users (Instruction 048):
- **Semantic HTML**: All interactive elements use semantic tags or proper `aria-role` definitions.
- **ARIA Labels**: Descriptive `aria-label` or `aria-labelledby` are applied to all icon-only buttons (Play, Pause, Favorite, Settings) and navigation sections.
- **Keyboard Navigation**: Focus management follows the visual flow, ensuring logical tab order across the sidebar, players, and command palette.
- **Interactive Boundaries**: Standardized components like `InteractiveArtwork` and `InteractiveTitle` prevent nested interactive elements (e.g., button within a link), resolving critical A11y warnings.
- **External Links**: All external links are marked with `target="_blank" rel="noopener noreferrer"` and descriptive screen reader text.
...
7. **Network Status & Offline Resilience** (`src/hooks/useNetworkStatus.ts`)
   - `useNetworkStatus()` - Debounced hook for tracking `isOnline` status.
   - **Policy**: Debounce of 1s is applied to both online and offline transitions to prevent fluttering and redundant toasts.
   - **Caveat**: The hook uses a module-level singleton listener; if multi-window or multi-app instances are introduced, ensure per-window state stays in sync and does not leak across instances.
   - **Error Handling**: `fetchUtils.ts` throws a specific `NetworkError` when offline.
   - **Toast Source**: Offline toast is emitted only on online → offline transition at the root layout; request-level handlers should not duplicate it.
   - **UI Pattern**: Explore collapses remote-only modules while keeping local entry points visible; remote search results are hidden in Command Palette.

8. **Tab Synchronization** (`src/hooks/useTabSync.ts`)
   - `useTabSync()` - Ensures singleton playback across tabs via `BroadcastChannel` with a `localStorage` fallback.
   - **Guardrails**: Ignores same-tab and stale events, and only pauses when the receiving tab is actively playing.

These helpers are widely reused as basic capabilities: ID generation, storage access, event listening, external link opening, time/size formatting, network status tracking, and tab synchronization are uniformly provided by the above modules.
...
4. **Interactive Artwork** (`src/components/interactive/InteractiveArtwork.tsx`)
   ...
   - **Trigger Binding**: Bind hover trigger to parent list row/card (e.g., `episode`/`item`/`session`/`card`) via `hoverGroup`.
    - **Play Button Visibility**: Only show play button when caller passes `onPlay`; cards or grids without direct playback capability (e.g., all Explore page modules) must not show play button to avoid misleading users. Navigation only modules should prioritize clarify and direct access to detail pages.
...
7. **Standardized Bits (Animations)** (`src/components/bits/`)
   - **`AnimatedList`**: Standard container for entrance animations. Uses a **data-driven API** (`items`, `renderItem`, `getKey`) to ensure stable keys and prevent layout flickering. Uses Framer Motion's `AnimatePresence` and `staggerChildren` to create a premium staggered slide-up effect.
...
8. **Legal Pages** (`src/routeComponents/legal/`)
   - Standardized layout for Privacy Policy and Terms of Service.
   - Uses `prose` classes for consistent content styling.
   - Entry points available in Settings Page.
...
- `lineHeight`: 1.4
- `paragraphMargin`: 10px (bottom)
- `whiteSpace`: `normal` (Follow standard typography flow)
