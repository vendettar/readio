...
`src/index.css` contains theme tokens, interaction class styles (reading/selection/drag-and-drop), Smart List Dividers, and local scrollbar styles. Layout dimensions use Tailwind tokens and extended tokens (`w-panel` 25rem, `w-panel-sm` 20rem, `max-w-content` 105rem/1680px, `max-w-content-wide` 112.5rem).

**Tailwind Layout Tokens** (via `tailwind.config.js`):
- **Spacing**: `sidebar`, `mini-player`, `page-gutter`, `page-margin` (all map to CSS variables).
- **Width**: `w-sidebar`.
...
- **Horizontal Spacing**: Page-level containers must use `px-[var(--page-margin-x)]` to ensure a 48px (3rem) safe distance from the sidebar and reserve space for the episode list Gutter animation.
...
**Key Rule**: Keep `toast.*Key` helpers. Key → text translation happens inside `src/lib/toast.ts` via `i18next.t` (or `translate()`), not at call sites.
...
5. **EpisodeRow & BaseEpisodeRow** (`src/components/EpisodeRow/`)
   - **BaseEpisodeRow**: Pure Presentation Component.
     - **Visual Specs**:
       - **Hover Background Layer**: Must use absolute positioning (`absolute inset-y-0`) with fixed left offset (`-left-[var(--page-gutter-x)]`). This ensures the selection highlight area for all list items (with or without cover) aligns perfectly as a rectangular column vertically.
