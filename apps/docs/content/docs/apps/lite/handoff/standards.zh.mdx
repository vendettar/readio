...
键盘快捷键集中在 `src/hooks/useKeyboardShortcuts.ts` 中，使用 `react-hotkeys-hook`。
- **Space**：切换播放/暂停（全局启用，但在输入框/文本域聚焦时禁用）。
- **Arrow Left/Right**：向后/向前跳转 15 秒。
- **Cmd/Ctrl + K**：启动全局搜索命令面板。
- **Esc**：全局优先关闭搜索覆盖层，或退出全屏播放器（沉浸模式）。

#### 手势（触摸）

移动端交互遵循原生播放器模式：
- **下滑关闭**：在 `FullPlayer.tsx` 中使用 `@use-gesture/react` 实现。从容器（包括顶部拖拽手柄）向下滑动超过 100px 触发 `exitImmersion()`。
- **拖拽手柄**：顶部 `top-2` 位置的可视化药丸形手柄提示下滑交互。组件设置 `pointer-events-none` 以不干扰点击/轻触。
...
- **间距 (Spacing)**: `sidebar`, `mini-player`, `player-footer`, `page-gutter`, `page-margin`（均映射到 CSS 变量）。
- **宽度 (Width)**: `w-sidebar`。
...
#### PWA 更新通知
当有新的 PWA 版本可用时，应用会显示一个**无限时长**的 Toast，并带有一个“立即刷新”的操作按钮。这由 `usePwaUpdate` hook 管理，该 hook 使用了 `vite-plugin-pwa` 提供的 `needRefresh` 状态。

#### 新手引导与空状态
空白页面被 `<EmptyState />` 组件取代，该组件提供：
- 大型视觉图标。
- 醒目的标题和描述性次要文本。
- 引导用户下一步操作的主调用按钮（CTA）（例如，“发现播客”或“添加音频”）。

#### 资源加载与兜底
- **图片兜底策略**：损坏的封面 URL 会自动被替换为居中的 Lucide `Podcast` 图标，并带有主题化的占位背景。
- **懒加载**：图片使用原生 `loading="lazy"` 或适当的 Framer Motion 入场动画。

#### Tooltip

Provider 位于 `src/main.tsx`（`Tooltip.Provider`）。

### 无障碍标准 (Immersive & Inclusive) [已实现]

应用遵循严格的无障碍标准，确保屏幕阅读器和键盘用户的使用体验（指令 048）：
- **语义化 HTML**：所有交互元素均使用语义化标签或正确的 `aria-role` 定义。
- **ARIA 标签**：为所有仅图标按钮（播放、暂停、收藏、设置）及导航区域应用描述性的 `aria-label` 或 `aria-labelledby`。
- **键盘导航**：焦点管理遵循视觉流向，确保侧边栏、播放器和命令面板具备逻辑性的 Tab 顺序。
- **交互边界**：通过 `InteractiveArtwork` 和 `InteractiveTitle` 等标准化组件防止嵌套交互元素（如链接内嵌套按钮），解决关键的 A11y 警告。
- **外部链接**：所有外部链接均标记有 `target="_blank" rel="noopener noreferrer"` 以及描述性的屏幕阅读器文本。
...
7. **网络状态与离线韧性** (`src/hooks/useNetworkStatus.ts`)
   - `useNetworkStatus()` - 带有去抖功能的网络状态追踪 Hook。
   - **策略**：在线和离线状态转换均应用 1s 去抖，防止网络瞬时波动导致 UI 闪烁或重复弹出 Toast。
   - **注意事项**：该 Hook 使用模块级单例监听；如引入多窗口或多实例运行模式，需要确保各实例的网络状态同步且不互相污染。
   - **错误处理**：`fetchUtils.ts` 在离线时抛出特定的 `NetworkError`。
   - **Toast 来源**：离线 Toast 仅在根布局的在线→离线转换时触发，请求级错误处理不应重复提示。
   - **UI 模式**：Explore 在离线时折叠远程模块并保留本地入口；Command Palette 隐藏远程搜索结果。

8. **标签页同步** (`src/hooks/useTabSync.ts`)
   - `useTabSync()` - 通过 `BroadcastChannel` + `localStorage` 回退实现单例播放。
   - **防护**：忽略同标签与过期事件，仅在当前标签处于播放中时触发暂停。

这套 helpers 作为基础能力被广泛复用：ID 生成、存储访问、事件监听、外链打开、时间/文件大小格式化、网络状态追踪与标签页同步统一由上述模块提供。
...
2. **溢出菜单** (`src/components/ui/overflow-menu.tsx`)
   ...
   - **必选默认值**：
     - 所有三点菜单必须使用 `sideOffset={8}` 与 `collisionPadding={16}`
     - 动作触发内联编辑（重命名、内联输入）时使用 `modal={false}`
     - 行/卡片上下文需在触发器与内容上 `stopPropagation`
   - **Overflow Hidden**：仅允许用于多面板滑动菜单（menu → confirm）。单面板菜单禁止使用 `overflow-hidden`。
...
4. **交互式封面 (InteractiveArtwork)** (`src/components/interactive/InteractiveArtwork.tsx`)
   ...
   - **播放按钮可见性**：仅当调用方传入 `onPlay` 时显示播放按钮；未提供直接播放能力的卡片或网格（如 Explore 页面的所有模块）不得显示播放按钮（避免误导）。导航模块应优先考虑清晰度，并引导用户进入详情页。
...
7. **标准化 Bits (动画)** (`src/components/bits/`)
   - **`AnimatedList`**: 入场动画的标准容器。采用**数据驱动 API** (`items`, `renderItem`, `getKey`) 以确保稳定的 Key 并防止布局抖动。使用 Framer Motion 的 `AnimatePresence` 和 `staggerChildren` 创建高级的交错上滑效果。
   - **`CountUp`**: 数字动画组件。使用 `useSpring` 实现平滑的数值过渡。主要用于统计和用量数值（如 Settings 存储信息）。

8. **法律页面 (Legal Pages)** (`src/routeComponents/legal/`)
   - 隐私政策与服务条款的标准布局。
   - 使用 `prose` 类保持内容样式的统一。
   - 入口位于设置页面。
...
- `lineHeight`: 1.4
- `paragraphMargin`: 10px (底部)
- `whiteSpace`: `normal` (遵循标准排版流)
