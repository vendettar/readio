...
To prevent unbounded IndexedDB growth, all playback history is subject to an automatic retention policy:
- **Limits**: Most recent **1000** sessions OR **last 6 months**, whichever is more restrictive.
- **Trigger**: Maintenance runs in a background task during app boot (see `retention.ts`).
- **Batching**: Deletions are processed in small batches to preserve UI responsiveness.
...
### Personal Vault (Metadata Export/Import) [Implemented]
- **Format**: Single JSON object with `version` and `exportedAt` at the top level.
- **Scope**: Metadata tables only (folders, tracks, subscriptions, favorites, history, settings). No audio/subtitle blobs.
- **Validation**: Uses Zod schema validation during import.
- **Integrity Check**: Performs business-level validation (Instruction 049):
  - **Dangling References**: Ensures subtitles point to existing tracks and tracks point to existing folders.
  - **UUID Uniqueness**: Prevents duplicate IDs within the imported dataset.
  - **Timestamp Sanity**: Rejects future timestamps (allowing for small clock skew).
  - **Deduplication**: Ensures no duplicate `feedUrl` or `key` entries within the dataset.
- **Logic**: Transactions ensure atomicity; overwrites existing metadata upon import.
- **MANDATORY**: Users MUST confirm via dialog before importing as it overwrites current state.
- **Warning**: Import is destructive and **not reversible**.

### OPML Import/Export [Implemented]
- **Import**: Parses OPML recursively (`outline` nodes), extracts `xmlUrl` + `title`.
- **Integrity**: Validates uniqueness of `xmlUrl` within the OPML file.
- **Dedupe**: Skips feeds that already exist by `feedUrl` unique index.
- **Storage**: Inserts subscriptions with UUID primary keys.
- **Export**: Generates OPML with escaped XML attributes for `title` and `xmlUrl`.
