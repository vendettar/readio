...
Uses Dexie.js to encapsulate IndexedDB. Default DB name is `readio` and can be overridden via `READIO_DB_NAME` (e.g., temporary reset names during wipe).
...
|-------|-------------|---------|------|
| sessions | id (UUID) | &trackId | Playback sessions (progress, audio/subtitle ID) |
...
| local_tracks | id (UUID) | folderId, createdAt | Local library audio tracks (links to audioBlobs) |
