...
**组件**：`src/components/Files/FileDropZone.tsx`

Files 页面支持使用 `react-dropzone` 进行拖拽文件上传。

**功能特性**：
- 包裹整个页面内容，支持在页面任何位置触发拖拽
- 使用 Zod Schema 进行文件校验 (`src/lib/schemas/files.ts`)
- 拖拽文件经过时显示毛玻璃效果覆盖层，并提示“放开以上传文件”
- 校验失败的文件会触发 Toast 错误提示

**校验规则**：
- **音频文件**：最大 500MB，必须是有效的音频 MIME 类型或扩展名 (`.mp3`, `.m4a`, `.wav`, `.ogg`, `.flac`, `.aac`, `.webm`)
- **字幕文件**：最大 5MB，必须是 `.srt` 或 `.vtt` 扩展名

**i18n Keys**：
- `dropzoneHint` — 拖拽区域提示语
- `dropzoneActive` — 拖拽激活（悬停）时的文字
- `validationFileTooLarge` — 文件超过大小限制的错误提示
- `validationInvalidAudioFormat` — 音频格式无效提示
- `validationInvalidSubtitleFormat` — 字幕格式无效提示
...
**元数据解析（尽力而为）**：
- **库**：`music-metadata-browser`（行业标准二进制解析器）。
- **执行**：在专用 Web Worker (`src/lib/files/metadata.worker.ts`) 中运行，防止大文件导入时阻塞主线程。
- **标题/时长**：优先使用元数据字段，而非文件名/音频元素估算值。
- 解析失败时回退到文件名 + 时长，不阻断导入。
...
## 4. Personal Vault（完整元数据导出/导入）

- **入口**：Settings → Personal Vault（导出/导入）。
- **范围**：仅元数据表（folders、local tracks/subtitles、subscriptions、favorites、playback sessions、settings），不包含音频/字幕 blob。
- **格式**：单一 JSON，包含 `version` 与 `exportedAt`。
- **导入安全**：覆盖前必须确认对话框。
- **校验**：写入前使用 Zod 校验；导入在 Dexie 事务内完成。
- **警告**：导入是破坏性的，**不可回滚**。
- **导入后同步**：导入完成后刷新订阅/收藏的内存缓存，确保 Settings 视图展示新数据。
