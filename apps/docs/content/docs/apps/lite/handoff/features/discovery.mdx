...
- **Degradation**: If offline (`!isOnline`), remote discovery modules collapse while keeping local entry points visible.
- **Hiding**: Remote charts (Top Shows, Editor's Picks, Top Episodes) are not shown to prevent empty states or error spam.
...
- **Context**: Subscriptions can ONLY be managed via the **Podcast Show Page**. The Explore page does not provide direct subscription actions.
...
- **Integrity**: Validates uniqueness of `xmlUrl` within the OPML file to prevent duplicate imports (Instruction 049).
- **Dedupe**: Import must skip existing `feedUrl` entries via unique index before insert.
- **OPML Dedupe**: Duplicate `xmlUrl` entries inside the OPML file are deduped (do not fail import).
- **Merge Semantics**: OPML import **merges** subscriptions (dedupe-only, no deletions).
- **Settings Export**: Settings must load subscriptions before export to avoid empty exports when Explore hasn't been opened.
