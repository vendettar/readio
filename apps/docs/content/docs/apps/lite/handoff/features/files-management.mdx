...
## 4. Personal Vault (Full Metadata Export/Import)

- **Entry**: Settings → Personal Vault (Export/Import).
- **Scope**: Metadata tables only (folders, local tracks/subtitles, subscriptions, favorites, playback sessions, settings). No audio/subtitle blobs.
- **Format**: Single JSON with `version` and `exportedAt`.
- **Import Safety**: Confirm dialog is mandatory before overwriting.
- **Validation**: Zod schema validation before any write; import runs inside a Dexie transaction.
- **Warning**: Import is destructive and **not reversible**.
- **Post-Import Sync**: After import, refresh in-memory subscriptions/favorites caches so Settings reflects the new vault contents.
