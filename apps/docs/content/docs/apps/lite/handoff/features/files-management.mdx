...
- If metadata parsing fails, fall back to filename + duration without blocking import.
...
- **Robust Metadata Parser**: Use `music-metadata-browser` in a Worker for ID3 parsing (Instruction 069).
