...
- **Offline Behavior**:
  - **Remote Results**: Suggestions, Podcast Results, and Episode Results are hidden when offline.
  - **Local Only**: Only "Local Library" matches (Favorites, Subscriptions, History, Files) are shown.
...
### Offline Behavior
- When `isOnline` is false, **hide remote results** (Suggestions, Remote Podcasts, Episodes) and keep local results only.
- Remote search does not execute; the UI surfaces local matches (Subscriptions, Favorites, History, Files) without delay.
- **Network Jitter**: Debounced online/offline changes (1s) and avoid repeated error toasts during rapid flaps.
- **Cache Miss**: If remote data is unavailable while offline, keep local-only mode without retry loops and do not show empty states for remote sections.
