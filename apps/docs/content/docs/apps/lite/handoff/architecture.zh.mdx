...
GlobalAudioController 通过 `useMediaSession` 集成媒体会话，向系统暴露播放控制与元数据，不引入新的播放来源。

该设计确保 `RootLayout` 在路由切换之外保持静态，播放过程中仅控制器与进度条组件更新。

### 后台性能（页面可见性）

- `usePageVisibility`（`src/hooks/usePageVisibility.ts`）监听 `document.visibilitychange`。
- `GlobalAudioController` 在隐藏时将 `updateProgress` 节流到 **≤ 1 秒/次**，同时继续处理音频事件。
- `FullPlayer` 在隐藏时暂停 CSS 与 Framer Motion 动画（不卸载）。
...
- **重复内容作用域 (Scoping)**：如果同一个逻辑对象（如某个播客）在同一页面中多次出现（例如在 Explore 页面的不同模块中），必须在 `layoutId` 前加上唯一的标识符前缀（如 `sectionId`）以防止冲突。跳转到详情页时，必须通过搜索参数 (`fromLayoutPrefix`) 传递此前缀，以便目标页面能匹配对应的起始元素。
...
- 通过 `useTabSync`（`src/hooks/useTabSync.ts`）实现，并挂载在 `GlobalAudioController`。
- 使用 `BroadcastChannel('readio_sync')`，在开始播放时发送包含 `senderId` 与 `timestamp` 的消息。
- 忽略当前标签发送的消息，忽略超过 2 秒的旧消息。
- 仅当当前标签处于播放中（`isPlaying === true`）时才触发暂停。
...
### 启动时维护 (Data Retention) [已实现]

为保持性能并防止元数据存储无限制增长，应用在启动序列中执行一项非阻塞维护任务（见 `useAppInitialization`）。

- **任务**：`prunePlaybackHistory` (`src/lib/retention.ts`)
- **策略**：保留最新的 **1000** 条会话 **或** 最近 **6 个月** 的记录，以限制更严格（更小）者为准。
- **实现**：使用分批删除策略（如每批 100 条），批次间有微小延迟，以确保主线程在初始加载期间保持对用户交互的响应。

### 资源集中管理 (ObjectURLs)

应用遵循严格的 transient 浏览器资源（Blob URLs）管理规则，以防止在单页应用环境中出现内存泄漏：

- **音频资源权威**：`playerStore` 是音频 ObjectURL 的**唯一**许可创建者和所有者。它维护一个 `activeBlobUrls` 数组，并确保在加载或恢复新音频源之前，显式调用 `URL.revokeObjectURL()` 释放之前追踪的所有 URL。
- **图像资源委托**：图像 ObjectURL 在组件级别使用 `useImageObjectUrl` Hook 进行管理。该 Hook 封装了 `create` 和 `revoke` 的生命周期，将 URL 的生命周期与组件（如 `TrackCard`、`InteractiveArtwork`）的挂载状态或源 Blob 的变化绑定。
- **严格隔离**：组件不再创建音频 URL。相反，它们将 IndexedDB 中的原始 Blob 传递给 Store Action（如 `loadAudioBlob()`）。
