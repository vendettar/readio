...
构建与测试脚本（根环境下执行）：`pnpm build`、`pnpm test`、`pnpm --filter @readio/lite test:run`、`pnpm --filter @readio/lite test:e2e`。
代码质量工具：Biome（`pnpm lint`、`pnpm format`）。

### CI 流水线 [已实现]

Readio 使用 GitHub Actions 进行持续集成。流水线在每次向 `main` 分支推送代码或提交 Pull Request 时运行：
- **Lint**: 确保代码符合 Biome 标准。
- **Typecheck**: 验证 TypeScript 类型。
- **单元测试**: 运行 Vitest 测试套件。
- **E2E 测试**: 运行 Playwright 烟雾测试。
...
Readio 已配置为高质量 PWA：
- **Manifest**：构建时由 `vite-plugin-pwa` 生成 `dist/manifest.json`（不要依赖 `public/manifest.json`）。
- **iOS 加固**：`index.html` 包含 Apple 相关 meta（`apple-mobile-web-app-capable`、`apple-touch-icon`）。
- **更新处理**：应用使用 `vite-plugin-pwa` 并配置 `registerType: 'prompt'`。当检测到新版本时，会显示一个 Toast，允许用户立即刷新并更新。
