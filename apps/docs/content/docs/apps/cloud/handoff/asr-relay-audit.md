...
- Provider verification/readiness now uses the same-origin relay for enabled providers; non-Groq providers remain disabled at runtime and unsupported by the backend until a later instruction explicitly expands that scope.
