...
- Network: only `fetchWithFallback` currently performs direct-first with proxy fallback; foreground prefetch still does direct `fetch` with `Range`; the native audio element remains browser-direct. Risk: over-broad retry logic would proxy requests that already work.
...
|---|---|---|---|---|---|---|---|---|
| Primary remote audio playback through native `<audio>` | `useAudioElementSync` via `GlobalAudioController` sets `audio.src` | Browser-direct media-element load; `blob:` when local download exists | Same as Lite | No | Not for native audio; Cloud UI does not render the proxy section | Not in `006a`; any later fallback here must use an explicit proxied media URL strategy | None in this audit phase | Native audio transport is not a JS retry target and must not be silently routed through a generic fetch fallback |
