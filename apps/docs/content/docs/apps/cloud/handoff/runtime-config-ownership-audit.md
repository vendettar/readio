...
| `READIO_ASR_RELAY_TOKEN` | backend `/env.js` allowlist (read from env var `READIO_ASR_RELAY_TOKEN`, Go constant `asrRelayTokenEnv` in `asr_relay.go:26`) | `apps/cloud-ui/src/lib/asr/backendRelay.ts` via `getAppConfig()` | lightweight abuse-control token; not a secret boundary |
...
- `READIO_ASR_RELAY_TOKEN` remains abuse-control only and must not be documented as a strong secret boundary.
