...
- **Decision**: Centralize the sleep timer countdown ticker in `GlobalAudioController.tsx`, while maintaining the state in a unified `useSleepTimerStore`.
- **Rationale**: Ensures the countdown persists across page navigation (since the controller is persistent) and prevents redundant counters or "speeding up" bugs when multiple hook instances are used in different components. The player should revert to a `paused` state when the timer expires.
