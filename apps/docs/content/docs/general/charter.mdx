...
2) **Handoff documentation must always match actual code**
   - After completing any task (feature/refactor/bugfix), update `apps/docs/content/docs/apps/lite/handoff/index.mdx` and the relevant sub-doc to reflect the current implementation (results only; no process notes).
