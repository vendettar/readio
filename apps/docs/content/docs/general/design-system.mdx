...
- If a fallback is needed, centralize it in a single helper (e.g. `src/lib/id.ts`) and use it everywhere.

### Storage & Cache Access
- Do NOT introduce new direct `localStorage.*` access in feature code.
- Route all new storage/cache reads/writes through a small helper layer (e.g. `src/lib/storage.ts`) so:
...
## Typography & Readability (New Section)
...
`docs/apps/lite/handoff.md` can remain a **skeleton/template during large migrations**.

However, after each task (feature/refactor/bugfix) is completed, the agent MUST:
- Update `docs/apps/lite/handoff.md` to match the **actual code state** (results only; no process notes).
...
## Data Sovereignty & Portability (The Personal Vault)
...
- **Philosophy**: Users own their data. The app is just a lens to view it.