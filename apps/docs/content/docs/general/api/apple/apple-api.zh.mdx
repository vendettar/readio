...
## PWA 与 iOS 集成（应用元数据 + 分享）

- **Manifest**：由 `vite-plugin-pwa` 生成为 `manifest.json`，唯一事实来源是 `apps/cloud-ui/vite.config.ts` 的 `VitePWA.manifest`。不要维护手动的 `apps/cloud-ui/public/manifest.json`。
- **iOS Meta 标签**：`apps/cloud-ui/index.html` 包含：
  - `apple-mobile-web-app-capable`
  - `apple-mobile-web-app-status-bar-style`
  - `apple-touch-icon`
- **原生分享**：`apps/cloud-ui/src/components/Player/ShareButton.tsx` 使用 `navigator.share` 传入标题与 URL，桌面端回退为剪贴板复制。
...
> **客户端过滤**: 
> 当搜索词存在拼写错误（如 `the kevin spacy trail`）或匹配度较低时，Apple Search API 有时会返回完全不相关的“热门”结果。
> 
> 为了提升用户体验，Readio 实施了**严格的客户端过滤**：
> - 我们将用户的查询分词并移除停用词（`the`, `podcast`, `it` 等）。
> - 我们检查结果的 `collectionName`, `trackName` 或 `artistName` 是否包含**至少一个**匹配的词。
> - 如果没有词匹配，则丢弃该结果。这防止了显示完全不相关的播客。
...
### ⚠️ 剧集排序与 Limit 不一致
**问题**: 对于更新频繁的播客（如 *NPR Up First*），iTunes `lookup` API 的剧集排序可能不一致。“最新”的单集可能不会出现在前 50 个结果中，即使它是今天发布的。
**影响**: 如果使用 `limit=50` 查询，可能会漏掉最新单集，导致用户尝试通过深层链接或排行榜访问时应用程序中出现“未找到剧集”错误。
**解决方案**: 在使用 `lookup` API 进行关键剧集解析（尤其是作为兜底逻辑）时，始终使用更高的限制（例如 `limit=300`）。
