...
### 3. `ActionToggle` Standard (`src/components/ui/action-toggle.tsx`)
For compact subscribe/favorite style controls with icon morph transitions, use `ActionToggle`.
- **Contract**: `active` state controls icon morphing, width collapse, label visibility, and aria label switching.
- **Icons**: pass explicit `activeIcon` and `inactiveIcon`; active icon uses filled treatment only when the consumer icon requires it.
- **No Motion Dependency**: transition behavior is CSS-only; do not add Framer Motion to this primitive.
- **Sizing**: use `size="default"` for page actions and `size="compact"` for denser list contexts.

### 4. `ExpandableDescription` Standard (`src/components/ui/expandable-description.tsx`)
Use `ExpandableDescription` for long show/episode descriptions that need shared truncation affordance.
- **Mode**:
  - `plain`: render `stripHtml(content, { preserveLineBreaks: true })`.
  - `html`: sanitize with `sanitizeHtml(content)` before HTML rendering.
- **Behavior**: preserve gradient fade + inline "show more" affordance when collapsed and link-style "show less" when expanded.
- **Control**: expansion state is controlled by the parent via `expanded` + `onExpandedChange`.
...
### 2. Standard Components (`apps/cloud-ui/src/components/ui/skeleton.tsx`)
- `Skeleton`: The base shimmer block.
- `PodcastCardSkeleton`: Matching `PodcastCard` dimensions.
- `EpisodeRowSkeleton`: Matching `EpisodeRow` layout.
- `ExploreHeroSkeleton`: For featured main carousels.

### 3. Usage Pattern
Replace generic loading spinners with structured skeletons to eliminate layout shifts.
```tsx
if (isLoading) return (
  <div className="space-y-4">
    {['s1', 's2', 's3'].map(key => <EpisodeRowSkeleton key={key} />)}
  </div>
);
```
