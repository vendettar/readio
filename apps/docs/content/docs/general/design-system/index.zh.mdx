...
- **内联样式（允许例外）**：仅允许用于数值驱动的 transform（translate/rotate/scale）或宽高比例的动态变化（例如进度条）。
...
- `.transcript-text`：`.prose` 的容器级修改器，将排版切换到阅读比例 (`--reading-*`) 并设置衬线字体栈。
- `.subtitle-text`：用于单个逐字稿行的细粒度排版类。
...
应用数据库中的所有持久化时间字段，必须统一使用 Unix 秒整数。

- 列名必须使用 `*_unix` 后缀。
- 列类型必须使用 `INTEGER`。
- 只保存原始时间戳数值，不允许并行保存 RFC3339 等格式化字符串列。
- 后端 API 响应和前端非 UI 层必须透传 Unix 秒数字。不要把 API payload 格式化为 RFC3339、ISO 字符串、本地化字符串或带时区语义的文本。
- 任何展示格式化只能发生在前端 UI 渲染边界。
- 外部 upstream 的时间字符串可以在集成入口边界解析；但数据一旦进入 Readio 内部模型、缓存、store、播放元数据、收藏、路由或测试夹具，节目发布时间必须保持 Unix 秒数字。
- 新 schema 不允许再引入 `created_at TEXT`、`updated_at TEXT`、`published_at TEXT` 等格式化时间列。
