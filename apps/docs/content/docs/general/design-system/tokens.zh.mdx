...
**间距 Tokens** (通过 `tailwind.config.js` 中的 `theme.extend.spacing`):
- `sidebar`: 映射到 `var(--sidebar-width)` (260px)。用于 `w-sidebar`, `pl-sidebar` 等。
- `mini-player`: 映射到 `var(--mini-player-height)` (73px)。用于 `h-mini-player`, `pb-mini-player` 等。
- `page-gutter`: 映射到 `var(--page-gutter-x)` (桌面 24px，移动 12px)。用于 `gap-page-gutter` 等。
- `page-margin`: 映射到 `var(--page-margin-x)` (桌面 48px，移动 24px)。按需使用。

**显式简写工具类**:
- **宽度**: `w-sidebar` (侧边栏宽度)。
- **高度**: `h-mini-player` (迷你播放器高度)。
- **内边距**: `p-page` / `px-page` / `p-gutter` / `px-gutter`。
- **外边距**: `m-page` / `mx-page` / `m-gutter` / `mx-gutter`。
