...
### 3. `ActionToggle` 标准 (`src/components/ui/action-toggle.tsx`)
对于订阅/收藏这类紧凑的图标形变操作，统一使用 `ActionToggle`。
- **契约**: 由 `active` 驱动图标形变、按钮宽度折叠、文案显隐以及 aria label 切换。
- **图标**: 显式传入 `activeIcon` 与 `inactiveIcon`；仅在业务图标需要时使用 active 填充视觉。
- **无 Motion 依赖**: 动画必须保持 CSS 过渡实现，不向该原语引入 Framer Motion。
- **尺寸**: 页面主操作使用 `size=\"default\"`，高密度列表可使用 `size=\"compact\"`。

### 4. `ExpandableDescription` 标准 (`src/components/ui/expandable-description.tsx`)
对需要统一截断交互的节目/单集长描述，统一使用 `ExpandableDescription`。
- **模式**:
  - `plain`: 渲染 `stripHtml(content, { preserveLineBreaks: true })`。
  - `html`: 先 `sanitizeHtml(content)` 再执行 HTML 渲染。
- **行为**: 折叠态保持渐变遮罩 + 内联“显示更多”，展开态保持“收起”链接按钮。
- **控制方式**: 展开状态由父组件通过 `expanded` + `onExpandedChange` 受控管理。
...
### 2. 标准组件 (`apps/cloud-ui/src/components/ui/skeleton.tsx`)
- `Skeleton`: 基础流光块。
- `PodcastCardSkeleton`: 匹配 `PodcastCard` 尺寸。
- `EpisodeRowSkeleton`: 匹配 `EpisodeRow` 布局。
- `ExploreHeroSkeleton`: 用于主页精选轮播。

### 3. 使用模式
使用结构化骨架屏代替通用的加载进度条，以消除布局抖动。
```tsx
if (isLoading) return (
