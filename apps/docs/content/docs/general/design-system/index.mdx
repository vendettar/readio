...
- **Zero Arbitrary Values**: Avoid Tailwind's arbitrary value syntax `-[...]` for static properties (colors, spacing, z-index, stroke-width). Register them in `tailwind.config.js` or use standard tokens (e.g., use `stroke-2` instead of `stroke-[3]`). New Tailwind tokens MUST be reused in 2+ locations; otherwise use standard classes.
- **Dynamic Values (Allowed)**: Bracket syntax is allowed ONLY for variables driven by runtime state or DOM measurements (e.g., `start-[var(--x)]`, `top-[var(--y)]`, `w-[var(--drag-preview-w)]`). **Never use this as a shortcut for static layout calculations.**
- **Inline styles (allowed exceptions)**: Inline styles are permitted only for numeric-driven transforms (translate/rotate/scale), dynamic width/height ratios (e.g., progress indicators), or cases where variables are consumed via `var(--...)` in CSS. Explicitly use `style={{ '--property': value }}` for complex dynamic layouts.
...
- `.transcript-text`: A container-level modifier for `.prose` that switches typography to the reading scale (`--reading-*`) and sets the serif stack.
- `.subtitle-text`: Granular typography for individual transcript lines.
- `.prose-isolate`: Applied to external RSS content (e.g. show notes) to revert inherited styles and isolate formatting.
...
- `z-sidebar` (40): Main navigation sidebar.
- `z-mini-player` (50): Mini playback controls (above sidebar).
- `z-overlay` (60): Tooltips and small hovering elements.
- `z-modal` (100): Full-screen players, dialogs, and sheets.
- `z-full-player` (100): Alias for `z-modal`.
- `z-menu` (150): Dropdowns, popovers, and context menus (highest layer).
...
- **Web frontend**: Validate with MIME type + file extension only.
...
## Persistence Time Rule

All persisted time fields in application databases are mandatory Unix-second integers.

- Column names must use the `*_unix` suffix.
- Column types must use `INTEGER`.
- Persist only the raw timestamp value. Do not store parallel formatted strings such as RFC3339 text columns.
- Backend API responses and non-UI frontend layers must pass time values through as Unix-second numbers. Do not format API payloads as RFC3339, ISO strings, locale strings, or timezone-specific text.
- Any display formatting must happen only at the frontend UI rendering boundary.
- External upstream time strings may be parsed at the integration boundary, but once data enters Readio's internal models, caches, stores, playback metadata, favorites, routes, or tests, episode publication times must remain Unix-second numbers.
- New schemas must not introduce `created_at TEXT`, `updated_at TEXT`, `published_at TEXT`, or similar formatted time columns.
