...
var (
	...
	errPodcastTranscriptAssetsDisabled = errors.New("podcast transcript assets are disabled until migration 00004 ships")
	podcastTranscriptAssetsEnabled     = false // TODO(00004): enable after podcast_transcript_assets migration is active.
)
...
func storePodcastTranscriptAsset(
	...
	if !podcastTranscriptAssetsEnabled {
		return nil, errPodcastTranscriptAssetsDisabled
	}
	...
	asset := &podcastTranscriptAsset{
		...
		CreatedAtUnix:          unixPodcastTranscriptCreatedAt(now),
	}
...
func loadPodcastTranscriptAssetByKey(
	...
	transcriptRoot string,
	transcriptKey string,
) (*podcastTranscriptAsset, *asr.ASRRelayResponsePayload, error) {
	if !podcastTranscriptAssetsEnabled {
		return nil, nil, errPodcastTranscriptAssetsDisabled
	}
	...
	rows, err := db.QueryContext(
		...
		WHERE itunes_id = ?
		  ...
		  AND ((? IS NULL AND audio_source_fingerprint IS NULL) OR audio_source_fingerprint = ?)
			ORDER BY created_at_unix ASC, transcript_key ASC`,
		prepared.itunesID,
		prepared.episodeKey,
		prepared.sourceKind,
...
func readPodcastTranscriptPayload(
	transcriptRoot string,
	asset *podcastTranscriptAsset,
) (*asr.ASRRelayResponsePayload, error) {
	...
	var payload asr.ASRRelayResponsePayload
...
func unixPodcastTranscriptCreatedAt(now time.Time) int64 {
	return now.UTC().Unix()
}
