...
export const Route = createFileRoute('/$country')({
  beforeLoad: ({ params, location }) => {
    ...
    if (!normalizedCountry) {
      debug('[country-route] Unsupported country parameter', {
    ...
    if (params.country !== normalizedCountry) {
      const canonicalPath = location.pathname.replace(/^\/[^/]+/, `/${normalizedCountry}`)
      debug('[country-route] Canonicalizing country parameter', {
        from: params.country,
