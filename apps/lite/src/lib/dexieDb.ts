...
function buildSchema(tracksSchema: string) {
  return {
    ...
    [DB_TABLE_NAMES.SUBSCRIPTIONS]: 'id, &feedUrl, addedAt, providerPodcastId, countryAtSave',
    [DB_TABLE_NAMES.FAVORITES]:
      'id, &key, addedAt, episodeId, providerEpisodeId, audioUrl, countryAtSave',
...
function normalizeSessionCountryAtSave(
  session: Pick<
    PlaybackSession,
    'source' | 'countryAtSave' | 'episodeId' | 'providerEpisodeId' | 'providerPodcastId'
  >,
  entityName: string
): string | undefined {
  ...
  const isNavigableExploreRecord =
    !!session.episodeId || !!session.providerEpisodeId || !!session.providerPodcastId
...
export const DB = {
  ...
  async getPlaybackSessionsByEpisodeId(episodeId: string): Promise<PlaybackSession[]> {
    if (!episodeId) return []
    return db.playback_sessions.where('episodeId').equals(episodeId).toArray()
  },
  ...
  async getSubscriptionsByProviderPodcastId(providerPodcastId?: string): Promise<Subscription[]> {
    if (!providerPodcastId) return []
    return db.subscriptions.where('providerPodcastId').equals(providerPodcastId).toArray()
  },
  ...
  async getFavoritesByEpisodeId(episodeId: string): Promise<Favorite[]> {
    if (!episodeId) return []
    return db.favorites.where('episodeId').equals(episodeId).toArray()
  },
  ...
  async findPlaybackSessionsByEpisodeShortId(shortId: string): Promise<PlaybackSession[]> {
    if (!shortId || shortId.length < 4) return []
    // Use startsWithIgnoreCase to match GUID prefixes stored in episodeId
    return db.playback_sessions.where('episodeId').startsWithIgnoreCase(shortId).limit(5).toArray()
  },
