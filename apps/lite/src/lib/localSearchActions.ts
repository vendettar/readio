...
import { generateSlugWithId } from './slugUtils'
...
function buildLibraryEpisodeRoute(params: {
  ...
  const podcastId = params.providerPodcastId?.trim()
  const rawEpisodeId = params.episodeId?.trim() || params.providerEpisodeId?.trim()

  if (!country || !podcastId || !rawEpisodeId) return null

  return buildPodcastEpisodeRoute({
    country,
    podcastId,
    episodeSlug: generateSlugWithId(params.episodeTitle?.trim() || 'episode', rawEpisodeId),
  })
}
