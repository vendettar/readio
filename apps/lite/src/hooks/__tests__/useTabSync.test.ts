...
describe('useTabSync', () => {
  ...
  it('uses storage event fallback when BroadcastChannel is unavailable', () => {
    ...
    expect(usePlayerStore.getState().isPlaying).toBe(false)
    expect(usePlayerStore.getState().status).toBe('paused')
    expect(localStorage.getItem('readio_sync')).toContain('"type":"PLAYING"')
  ...
  it('cleans fallback storage timer on unmount without delayed side effects', () => {
    ...
    const { unmount } = renderHook(() => useTabSync())
    ...
    localStorage.setItem('readio_sync', 'manual-persist')

    act(() => {
      vi.advanceTimersByTime(60)
    })
    expect(localStorage.getItem('readio_sync')).toBe('manual-persist')
  })
})
