...
export function FileDropZone({
  ...
  const onDrop = useCallback(
    (acceptedFiles: File[], rejectedFiles: FileRejection[]) => {
      ...
      const getErrorMessage = (message: string | undefined): TranslationKey => {
        const keyMap: Record<string, TranslationKey> = {
          validationFileTooLarge: 'validationFileTooLarge',
          validationInvalidAudioFormat: 'validationInvalidAudioFormat',
          validationInvalidSubtitleFormat: 'validationInvalidSubtitleFormat',
        }
        return (message && keyMap[message]) || 'toastFileValidationError'
      }
      ...
      for (const file of acceptedFiles) {
        if (isValidAudioFile(file)) {
          ...
          if (result.success) {
            ...
            toast.errorKey(getErrorMessage(result.error.issues[0]?.message))
          }
      ...
      if (validFiles.length > 0) {
        void Promise.resolve(onFilesAccepted(validFiles)).catch((err) => {
          logError('[Files] Failed to process dropped files', err)
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    noKeyboard: true, // Don't open file dialog on keypress (space/enter)
    accept: {
  ...
  return (
    <div
      ...
      data-file-dropzone-enabled={!disabled ? 'true' : 'false'}
    >
      ...
      {isDragActive && (
        <div className="absolute inset-0 flex items-center justify-center rounded-xl border-2 border-dashed border-primary bg-background/80 backdrop-blur-md">
