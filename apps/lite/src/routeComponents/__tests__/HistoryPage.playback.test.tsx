...
vi.mock('../../lib/player/remotePlayback', async (importActual) => {
  ...
  return {
    ...actual,
    playHistorySessionWithDeps: vi
      .fn()
      .mockImplementation(actual.playHistorySessionWithDeps as (...args: unknown[]) => unknown),
...
describe('HistoryPage playback session wiring', () => {
  ...
  it('keeps remote history resume semantics for sessionId and fileTrackId', async () => {
    render(<HistoryPage />)
    fireEvent.click(screen.getByLabelText('btnPlayOnly'))

    await waitFor(() => expect(setAudioUrlMock).toHaveBeenCalled())
    // setPlaybackTrackId is called only if a local track is resolved by the repo.
