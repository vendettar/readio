import type { Episode, Podcast } from '@readio/core'
...
vi.mock('@tanstack/react-query', () => ({
  useQuery: ({ queryKey }: { queryKey: readonly unknown[] }) => {
    ...
    if (queryKey[1] === 'feed') {
      return {
        data: {
          title: mockPodcast?.collectionName ?? '',
          description: '',
          artworkUrl: mockPodcast?.artworkUrl600,
...
describe('PodcastEpisodesPage virtualized grouped rendering', () => {
  ...
  it('renders grouped year headers, keeps row wiring, and preserves isLast at group boundaries', () => {
    ...
    expect(playEpisodeMock).toHaveBeenCalledWith(mockEpisodes[0], mockPodcast, undefined)
