# Instruction 003: Cloud Cloud UI Full Clone Bootstrap [COMPLETED]

## Objective
Create the first reliable Cloud bootstrap by making `apps/cloud-ui` a full copy-equivalent of `apps/cloud-ui`, then make `apps/cloud-api` serve that frontend and run the Cloud app on port `8080`.
...
It does **not** try to redesign or partially recompose Cloud UI into Cloud.

The immediate goal is:

- `apps/cloud-ui` must become a true Cloud UI-equivalent frontend
- Cloud must run through a Go backend entrypoint on `8080`
- frontend parity is more important than elegance in this phase

## Product Rule
For this bootstrap phase, treat Cloud as a deployment wrapper around the Cloud UI frontend product.

That means:

- `apps/cloud-ui` should be the same frontend product as `apps/cloud-ui`
- same routes
- same pages
- same UI
- same settings
...
If the frontend is not functionally identical to Cloud UI after this instruction, the instruction is incomplete.
...
1. copy the Cloud UI frontend product into `apps/cloud-ui`
...
- removing Cloud UI browser-direct networking
...
- `apps/cloud-ui` is effectively a relocated Cloud UI frontend
...
This bootstrap is allowed to keep Cloud UI networking behavior for now.

## Required Work

### Phase 1: Clone Cloud UI Frontend into Cloud UI
Use `apps/cloud-ui` as the source of truth.

Required rules:

- copy the frontend product code from `apps/cloud-ui` into `apps/cloud-ui`
...
- decide on your own that a Cloud UI feature is unnecessary in Cloud
...
- replacing real Cloud UI features with mock data surfaces
...
- rebuild Cloud UI networking contracts in Go during this step
...
The frontend should render as a Cloud UI-equivalent app through the Go-hosted origin.

## Explicit Non-Goals
This instruction must **not** be interpreted as:

- "start extracting shared primitives first"
- "make Cloud approximately match Cloud UI"
...
- `apps/cloud-ui` is still missing Cloud UI pages or behaviors
- `apps/cloud-ui` still contains placeholder replacements for Cloud UI surfaces
- the command palette is missing
- IndexedDB/Dexie-backed frontend behavior was dropped without explicit approval
- the settings page differs from Cloud UI
- the route surface differs from Cloud UI
...
- first establish a true Cloud UI-equivalent Cloud baseline
