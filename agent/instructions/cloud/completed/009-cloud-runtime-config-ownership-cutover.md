...
### Allowlist / Schema parity rule

Do not allow the backend `/env.js` allowlist and the frontend runtime schema to drift into two unrelated config tables.

This instruction must define and preserve an explicit parity rule between:

- `apps/cloud-api` browser-public emitted keys
- `apps/cloud-ui/src/lib/runtimeConfig.schema.ts`
- `apps/cloud-ui/src/lib/runtimeConfig.ts`

Preferred contract:

- one side is treated as the effective source of truth for browser-public runtime fields
- the other side is checked by a focused parity test or equivalent explicit sync mechanism

The worker must not leave the repo with two silent, hand-maintained field lists and no verification path between them.
...
It must be written into a reviewable artifact, not left implicit in code diffs.

At minimum the artifact must include, per field:

- field name
- current source
- target ownership bucket
- whether it remains browser-visible
- whether it continues to appear in `window.__READIO_ENV__`

Preferred location:

- Cloud handoff/runtime-config audit doc
- or an explicit section added to the relevant Cloud handoff doc for this cutover
...
- sets an explicit cache policy suitable for runtime config freshness (`Cache-Control: no-store` or an explicitly justified equivalent revalidation policy)
...
5. Keep `READIO_ASR_RELAY_PUBLIC_TOKEN` understood as a lightweight abuse-control token, not a true secret boundary.
...
Backend test coverage must not rely only on broad `go test ./...` execution.
This cutover must add or update focused backend tests for the `/env.js` handler proving:

- route ownership before SPA fallback
- expected browser-safe field emission
- exclusion of backend-only fields
- response headers for JavaScript content type and cache behavior
...
2. targeted `apps/cloud-api` tests covering `/env.js` ownership cutover behavior
   - include the focused `/env.js` handler test file(s) added/updated by this instruction
