---
description: Contract for the MiniPlayer 3-dot more menu, including transcript import/export behavior
---

# Instruction 021: MiniPlayer More Menu Logic

# Task: 为 Cloud MiniPlayer 增加稳定的 transcript More 菜单合同
...
2. 导出当前 transcript（first-pass 支持多格式）
...
4. 导出 transcript 时绕开既有 repository / export contract
...
1. `Export Transcript (.srt)`
2. `Export Transcript (.vtt)`
...
6. export transcript submenu exposes only approved transcript formats
7. import transcript success aligns current CTA/transcript-visible path with `019/020`
8. menu does not break MiniPlayer height / layout / z-layer contract
9. importing transcript while built-in remote transcript exists makes the imported transcript active for the current session
10. menu closes before file picker / action flow and disabled items produce no side effect
...
4. export transcript submenu 是否只暴露已批准的 transcript 格式动作
5. 是否与 `019/020` 的 transcript-first / CTA 语义保持一致
6. 是否遵守了与 Files / Downloads 现有 3-dot 菜单一致的 overflow-menu 结构与分组习惯
...
4. 大规模重写下载体系
5. 在 MiniPlayer 中提供 audio export 或 transcript+audio bundle export
...
- 手工验证：
  ...
  - transcript export submenu 是否只显示批准的格式动作
