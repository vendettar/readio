...
export function useSession() {
        ...
        const initSession = async () => {
            try {
                ...
                if (lastSession && lastSession.progress > 0) {
                    ...
                    log('[Session] Restored playback session:', lastSession.id, 'progress:', lastSession.progress);
    ...
    useEffect(() => {
        ...
        const findOrStartSession = async () => {
            try {
                ...
                if (existingSession) {
                    ...
                    log('[Session] Found previous session:', existingSession.id, 'progress:', existingSession.progress);
        ...
        findOrStartSession();
    }, [audioLoaded, subtitlesLoaded, duration, storeSessionId, localTrackId, setStoreSessionId, setProgress]);
