...
import { DB, type FileTrack, type FileSubtitle } from '../libs/dexieDb';
...
export function useFilePlayback({ onComplete }: UseFilePlaybackProps = {}) {
     ...
     */
    const handlePlay = useCallback(async (track: FileTrack, availableSubtitles: FileSubtitle[], subtitle?: FileSubtitle) => {
        const { setAudioUrl, setSubtitles: setPlayerSubtitles, play, setFileTrackId, setSessionId: setStoreSessionId } = usePlayerStore.getState();
     ...
     */
    const handleSetActiveSubtitle = useCallback(async (trackId: number, subtitleId: number) => {
        try {
