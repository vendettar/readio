...
import { DB, type FileFolder, type FileTrack, type FileSubtitle } from '../libs/dexieDb';
...
export function useFilesData(): UseFilesDataReturn {
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            let folder: FileFolder | undefined = undefined;
            if (folderId !== null) {
            ...
            for (const session of sessions) {
                if (session.audioId && session.lastPlayedAt) {
                    ...
                    if (!lastPlayedMap[session.audioId] || lastPlayedMap[session.audioId] < session.lastPlayedAt) {
