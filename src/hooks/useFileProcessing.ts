...
import { ingestFiles, attachSubtitleToTrack } from '../libs/files/ingest';
...
export function useFileProcessing({ currentFolderId, onComplete }: UseFileProcessingOptions) {
    const handleAudioInputChange = useCallback(async (
    ...
    const handleSubtitleInputChange = useCallback(async (
        e: React.ChangeEvent<HTMLInputElement>,
