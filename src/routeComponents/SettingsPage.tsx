...
import { Trash2, Eraser, Info } from 'lucide-react';
...
export default function SettingsPage() {
    ...
    const { deleteSession, clearSessionCache, wipeAll, isClearing } = useStorageMaintenance({ reload, t });
                                        ...
                                        {ACCENT_OPTIONS.map((a) => (
                                            <Button
                                                ...
                                                className={cn(
                                                    ...
                                                    accent === a.name ? "ring-2 ring-offset-2 ring-primary scale-110" : "opacity-70 hover:opacity-100"
                                                )}
                                                title={a.name}
                                            >
                    ...
                    <div className="space-y-6">
                        ...
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>{t('settingsStorageUsageDesc', { size: formatBytes(storageInfo?.indexedDB.totalSize ?? 0) })}</CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-6">
                                {storageInfo?.browser && (
                                    <div className="space-y-2">
                                        <div className="flex justify-between text-sm">
                                            ...
                                            <span className="text-muted-foreground">
                                                {formatBytes(storageInfo.browser.usage)} / {formatBytes(storageInfo.browser.quota)}
                                            </span>
                                        </div>
                                                ...
                                                <div key={item.id} className="p-3 flex items-center justify-between hover:bg-muted/50 transition-colors">
                                                    <div className="min-w-0 flex-1 pr-4">
                                                        ...
                                                        <div className="text-xs text-muted-foreground flex gap-2 mt-0.5">
                                                            ...
                                                            <span>•</span>
                                                            <span className="capitalize">{item.source === 'local' ? t('sourceLocal') : item.source === 'gallery' ? t('sourcePodcast') : item.source}</span>
                                                            <span>•</span>
                                                            <span>{formatBytes(item.sizeBytes)}</span>
                                                            {item.hasAudioBlob ? (
                                                                <span className="text-primary font-medium tracking-tight">{t('settingsSaved')}</span>
                                                            ) : (
