...
import { ChevronRight, Play, Plus, Check } from 'lucide-react';
import { Button } from '../../components/ui/button';
import { useI18n } from '../../hooks/useI18n';
import { useExploreStore } from '../../store/exploreStore';
import { lookupPodcastFull, fetchPodcastFeed } from '../../libs/discoveryProvider';
                        ...
                        <div className="space-y-5">
                            <div>
                                ...
                                <div className="flex items-center gap-2 mt-2 text-xs font-medium text-muted-foreground tracking-tight">
                                    <span>{podcast.primaryGenreName || podcast.genres?.[0] || t('podcastLabel')}</span>
                                </div>
                            </div>
                                ...
                                <div className={cn(
                                    ...
                                    <div className={cn(
                                        ...
                                        <Plus className={cn(
                                            ...
                                            isSubscribed ? "opacity-0 rotate-90 scale-0" : "opacity-100 rotate-0 scale-100"
                                        )} />
                                        <Check className={cn(
                                            "w-5 h-5 stroke-[3] absolute inset-0 m-auto transition-all duration-300",
                                            isSubscribed ? "opacity-100 rotate-0 scale-100" : "opacity-0 -rotate-90 scale-0"
                                        )} />
                                    </div>
                    ...
                    {isLoadingFeed ? (
                        <div className="space-y-3">
                            {Array.from({ length: 5 }).map((_, i) => (
                                <div key={i} className="p-4 rounded-lg bg-muted/50 animate-shimmer">
                        ...
                        <div className="flex flex-col">
                            ...
                            {feed?.episodes && feed.episodes.length > 8 && (
                                <div className="pt-2">
                                    <Button asChild variant="link" className="text-primary font-medium text-sm p-0 h-auto">
                                        <Link
                                            to="/podcast/$id/episodes"
                                            params={{ id }}
                                        >
                                            {t('seeAll', { count: formatCompactNumber(podcast.trackCount || feed.episodes.length) })}
                                        </Link>
                                    </Button>
                                </div>
                            )}
