...
import { lookupPodcastFull, fetchPodcastFeed, lookupPodcastEpisodes, lookupEpisode, type Episode, type SearchEpisode, type Podcast } from '../libs/discoveryProvider';
...
export default function ExplorePage() {
    ...
    const { data: topSubscriberShows, isLoading: isLoadingSubscribers } = useTopSubscriberPodcasts('us', 30);
    ...
    const handlePlayEpisode = async (episode: DiscoveryPodcast) => {
        ...
        try {
            ...
            if (!matchedEpisode?.episodeUrl) {
                ...
                matchedEpisode = results.find(ep =>
            ...
            if (matchedEpisode?.episodeUrl) {
                ...
                setAudioUrl(matchedEpisode.episodeUrl, matchedEpisode.trackName, coverArt, {
                    ...
                    podcastFeedUrl,
                    artworkUrl: coverArt,
                    publishedAt: matchedEpisode.releaseDate ? new Date(matchedEpisode.releaseDate).getTime() : undefined,
                    duration: matchedEpisode.trackTimeMillis ? Math.round(matchedEpisode.trackTimeMillis / 1000) : undefined,
                });
                play();
                if (episode.url) openExternal(episode.url);
            }
    ...
    const handleSubscribePodcast = async (podcast: DiscoveryPodcast) => {
        try {
            ...
            if (fullPodcast) {
                const subscriptions = useExploreStore.getState().subscriptions;
                const isSubscribed = subscriptions.some((s: Subscription) => s.feedUrl === fullPodcast.feedUrl);
    ...
    const handleToggleFavoriteEpisode = async (episode: DiscoveryPodcast) => {
        ...
        try {
            ...
            const favorited = favorites.find(f =>
                f.episodeId === episode.id ||
                f.audioUrl === episode.url
            );
            ...
            if (!fullEpisode) {
                ...
                fullEpisode = itunesEpisodes.find(ep =>
                    String(ep.trackId) === episode.id ||
                    ep.trackName.toLowerCase().includes(episode.name.toLowerCase())
                ) || null;
            }
                ...
                if (podcastToUse?.feedUrl) {
                    try {
                        ...
                        const rssEpisode = feed.episodes.find(ep => {
                            ...
                            return epTitle === apiTitle || epTitle.includes(apiTitle) || apiTitle.includes(epTitle);
                        });
