...
export async function validateFeedFetchable(country: string, feedUrl: string, signal?: AbortSignal): Promise<boolean> {
...
export async function pickCorsAllowedRecommended(
    ...
    for (const item of items) {
        ...
        if (status === true) { picked.push(item); seenFeeds.add(feedKey); }
        else if (status === null) { unknown.push(item); }
    }
    ...
    for (const item of unknown) {
        ...
        if (ok) { picked.push(item); seenFeeds.add(item.feedUrl.toLowerCase()); }
    }
