...
import { getJsonWithTtl, setJsonWithTtl, nsKey, getJson } from '../storage';
...
export function readLookupCache(country: string): Record<string, RecommendedPodcast> | null {
    ...
    const { CHART_CACHE_TTL_MS } = getConstants();
    const result = getJsonWithTtl<{ entries: Record<string, RecommendedPodcast> }>(key, CHART_CACHE_TTL_MS);
    if (!result) return null;
...
export function readFetchabilityCache(country: string): Record<string, { ok: boolean; at: number }> {
    ...
    Object.entries(cache).forEach(([url, entry]) => {
        ...
        if (item && typeof item === 'object' && typeof item.ok === 'boolean' && typeof item.at === 'number') {
