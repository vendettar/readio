...
import { setJsonWithTtl, nsKey, getJsonWithTtl } from './storage';
...
export async function fetchTopPodcasts(country = 'us', limit = 30, signal?: AbortSignal): Promise<DiscoveryPodcast[]> {
    ...
    return deduplicatedFetch(getRequestKey(url), async (fetchSignal) => {
        const data = await fetchJsonWithFallback<{ feed?: { results?: unknown[] } }>(url, { signal: signal || fetchSignal });
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export async function fetchTopEpisodes(country = 'us', limit = 30, signal?: AbortSignal): Promise<DiscoveryPodcast[]> {
...
export async function fetchTopSubscriberPodcasts(country = 'us', limit = 30, signal?: AbortSignal): Promise<DiscoveryPodcast[]> {
...
export async function searchPodcasts(query: string, country = 'us', limit = 30, signal?: AbortSignal): Promise<Podcast[]> {
    ...
    const { BASE_URLS } = getConfig();
    const params = new URLSearchParams({ term: cleanQuery, country, media: 'podcast', limit: String(limit) });
    const url = `${BASE_URLS.SEARCH}?${params}`;

    return deduplicatedFetch(getRequestKey(url), async (fetchSignal) => {
        const data = await fetchJsonWithFallback<{ results?: unknown[] }>(url, { signal: signal || fetchSignal });
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export async function searchEpisodes(query: string, country = 'us', limit = 20, signal?: AbortSignal): Promise<SearchEpisode[]> {
    const cleanQuery = query.toLowerCase().trim();
    if (!cleanQuery) return [];
...
export async function lookupPodcastsByIds(ids: string[], country = 'us', signal?: AbortSignal): Promise<DiscoveryPodcast[]> {
...
export async function lookupPodcastFull(id: string, country = 'us', signal?: AbortSignal): Promise<Podcast> {
 ...
 */
export async function lookupEpisode(trackId: string, country = 'us', signal?: AbortSignal): Promise<SearchEpisode | null> {
 ...
 */
export async function lookupPodcastEpisodes(podcastId: string, country = 'us', limit = 20, signal?: AbortSignal): Promise<SearchEpisode[]> {
    const cacheKey = `lookupPodcastEps:${country}:${podcastId}:${limit}`;
    const cached = getCache<SearchEpisode[]>(cacheKey);
    if (cached) return cached;

    const { BASE_URLS } = getConfig();
    const params = new URLSearchParams({ id: podcastId, country, entity: 'podcastEpisode', limit: String(limit) });
    const url = `${BASE_URLS.LOOKUP}?${params}`;
...
function parseRssXml(xmlText: string): ParsedFeed {
    ...
    const artworkUrl = getITunesTag(channel!, 'image')?.getAttribute('href') ||
    ...
    const episodes: Episode[] = items.map((item): Episode | null => {
        ...
        return {
            ...
            description: (getITunesTag(item, 'summary')?.textContent ||
            ...
            link,
            fileSize: isNaN(fileSize!) ? undefined : fileSize,
            transcriptUrl,
            chaptersUrl,
        };
