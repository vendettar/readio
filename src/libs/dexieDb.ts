...
class ReadioDB extends Dexie {
    ...
    constructor() {
        ...
        this.version(1).stores({
            playback_sessions: 'id, lastPlayedAt, source, createdAt, audioUrl, audioFilename, localTrackId, episodeId',
...
export const DB = {
    ...
    // ========== Utility ==========
    async getStorageStats(): Promise<{ sessions: number; audioBlobs: number; subtitles: number; totalSize: number }> {
        ...
        const totalSize = audioBlobs.reduce((sum, a) => sum + a.size, 0) +
    ...
    async updateFolder(id: number, data: Partial<Pick<FileFolder, 'name' | 'pinnedAt'>>): Promise<void> {
    ...
    async getFileTracksInFolder(folderId: number | null): Promise<FileTrack[]> {
        if (folderId === null) {
            return db.local_tracks.filter(t => t.folderId === null || t.folderId === undefined).toArray();
        }
