 ...
 */
export function formatRelativeTime(dateStr: string, t: (key: string, options?: Record<string, string | number>) => string): string {
    if (!dateStr) return '';
 ...
 */
export function formatDuration(seconds: number | undefined, t: (key: string, options?: Record<string, string | number>) => string): string {
    if (!seconds) return '';
...
export function formatTimeSmart(timestamp: number | string | Date, language: string): string {
    ...
    const systemHour12 = new Intl.DateTimeFormat(undefined, { hour: 'numeric' }).resolvedOptions().hour12;
