...
export function InteractiveArtwork({
    ...
    const hoverOpacityClass = hoverGroup ? hoverOpacityClassMap[hoverGroup] : 'group-hover/artwork:opacity-100';
    const hoverScaleClass = hoverGroup ? hoverScaleClassMap[hoverGroup] : 'group-hover/artwork:scale-110';

    const overlayClassName = cn(
        'absolute inset-0 bg-foreground/20 opacity-0 transition-opacity duration-200 pointer-events-none',
        hoverOpacityClass,
        playPosition === 'center' ? 'flex items-center justify-center' : 'flex items-end justify-start p-2'
    );

    const resolvedPlayIconSize = playIconSize ?? playIconSizeMap[size];

    return (
        <div className={cn("relative group/artwork overflow-hidden rounded-lg bg-muted shadow-sm flex-shrink-0", sizeClasses[size], className)}>
