...
import { Copy, Search, Book, X } from 'lucide-react';
...
export function LookupPopover({ state, onClose }: LookupPopoverProps) {
    ...
    return (
        <>
            <div className="fixed inset-0 z-[1100] bg-foreground/20 animate-in fade-in" onClick={onClose} />
            <div
            ...
            >
                ...
                <div className="p-4 overflow-y-auto flex-1">
                    {state.lookupLoading && (
                        <div className="py-8 text-center text-sm text-muted-foreground animate-pulse">{t('loading')}</div>
                    )}
        ...
        <div className="space-y-4">
            ...
            {entry.meanings.map((meaning, idx: number) => (
                <div key={idx} className="space-y-2">
                    <div className="text-xs font-black uppercase tracking-widest text-primary/60">{meaning.partOfSpeech}</div>
                    <ol className="space-y-3">
                        {meaning.definitions.map((def, defIdx: number) => (
                            <li key={defIdx} className="text-sm space-y-1">
                                ...
                                {def.example && (
                                    <div className="text-xs italic text-muted-foreground bg-muted/30 p-2 rounded-md border-l-2 border-primary/20">"{def.example}"</div>
                                )}
                            </li>
                        ))}
                    </ol>
