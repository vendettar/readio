...
import { FileAudio, FileType, Play, Trash2, Plus, Clock, Check, X, Package, Lock } from 'lucide-react';
import { useDraggable } from '@dnd-kit/core';
import { CSS } from '@dnd-kit/utilities';
import { cva } from 'class-variance-authority';
import type { FileTrack, FileSubtitle, FileFolder } from '../../libs/dexieDb';
...
const trackCardContentVariants = cva(
...
const subtitleSectionVariants = cva(
...
const subtitleIconContainerVariants = cva(
...
export function TrackCard({
    ...
    const handleConfirmRename = (isBlur = false) => {
        ...
        const isConflict = existingTrackNames.some(
            name => name.trim().toLowerCase() === trimmed.toLowerCase() && name.trim().toLowerCase() !== track.name.trim().toLowerCase()
        );
    ...
    return (
        <div
            ...
            className={cn(
                ...
                isDragging ? 'shadow-xl ring-2 ring-primary cursor-grabbing' : 'border-border hover:shadow-md',
                disableInteractions && 'pointer-events-none'
            )}
        >
            {/* Main Track Info */}
            <div
                className={trackCardContentVariants({ density })}
                {...listeners}
                {...attributes}
            >
                    ...
                    <div className="divide-y divide-border/50 mb-2">
                        {subtitles.map((sub) => (
                            <div key={sub.id} className={subtitleRowVariants({ density })}>
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2">
                                        <span className="text-sm font-medium text-foreground block truncate">{sub.name}</span>
